...
public class ConfigDAO extends BaseHibernateDAO {
	...
	public Config findById(java.lang.Integer id) {
		...
		try {
			Config instance = (Config) getSession().get("dao.Config", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Config> findByExample(Config instance) {
		...
		try {
			List<Config> results = (List<Config>) getSession()
					.createCriteria("dao.Config").add(create(instance)).list();
	...
	public List<Config> findBySystemName(Object systemName) {
